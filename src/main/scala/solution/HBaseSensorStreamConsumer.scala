...
object HBaseSensorStreamConsumer extends Serializable {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length < 2) {
      throw new IllegalArgumentException("You must specify the topic and table, for example /user/user01/pump:sensor  /user/user01/db/sensor")
    }
    ...
    System.out.println("Save to table : " + tableName)


    val brokers = "maprdemo:9092" // not needed for MapR Streams, needed for Kafka
    val groupId = "SparkApplicationHBase"
    ...
    jobConfig.set("mapreduce.output.fileoutputformat.outputdir", "./out")
